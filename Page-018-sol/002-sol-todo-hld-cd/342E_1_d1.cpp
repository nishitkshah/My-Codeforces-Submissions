...
int n, m, sqm, x, y;
int t[N_], v[N_], d[N_], dpt[N_], pw[N_+1], pp[N_+1], vis[N_];
...
while(lca[x][0]!=lca[y][0]){
	lv=min(lv, (int)min(lca[x].size()-1, lca[y].size()-1));
	while(lca[x][lv]==lca[y][lv]) lv--;
	x = lca[x][lv], y = lca[y][lv];
}
return lca[x][0];
}
...
for(int i=0; i<m; i+=sqm){
	...
	RE(j, i, i+sqm){
		...
		if(t[j]==1){
			...
			int ans = d[v[j]];
			RE(ri, rdsz, rd.size()){
				int rt = getlca(v[j], rd[ri]);
				ans = min(ans, dpt[v[j]]+dpt[rd[ri]]-2*dpt[rt]);
			}
			printf("%d\n", ans);
		}
	}
