...
inline long long query(int x, int y, int ind=0, int l=0, int r=N_-1){
//cerr << "q dbggg: " << x << ' ' << y << '\n';
propagate(ind);
...
inline void update(long long v, int x, int y, int ind=0, int l=0, int r=N_-1){
propagate(ind);
if(x==l && y==r){
	lz[ind] += v;
	return;
